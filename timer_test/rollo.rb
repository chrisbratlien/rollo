  ...
  def go(now = 0)  #now is the offset from start... i.e. the current time position         
    ...
    (1..4).each do |measure|
      (0..$bpmeasure-1).each do |step|
        ...
        dur = @tick[@now,$bpm,1] - @now
        
        @play[:notes => collect_for_this_step,
              :duration => dur,
              :wheen => @now,
              :timer => @timer,
              :measure => measure,
        ...
        now = Time.now.to_f
        diff = latest - now
        puts "LAST EVENT SCHEDULED: #{latest} NOW: #{now} DIFF: #{diff}"
        if diff > 0
          sleep(diff)
        end
        ...
        #puts @timer
        #@play[collect_for_this_step,$base_duration,@now,@timer,measure,step]
        @now = @tick[@now,$bpm,1]
        #puts @now
      end
    end
    ...
    #puts "START: #{@start} NOW: {@now}  START+NOW: #{@start+now}  Time.now: #{Time.now}"
