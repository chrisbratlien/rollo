...
LOG_PLAYBACK
      ...
      def sync_start(on_time = @clock.time)
        on_time += @clock.start
        @timer.at(on_time) {@interface.message(0xFA)}
      end

      def sync_stop(on_time = @clock.time)
        on_time += @clock.start
        @timer.at(on_time) {@interface.message(0xFC)}
      end
      
      def sync_clock(on_time = @clock.time)
        on_time += @clock.start
        @timer.at(on_time) {@interface.message(0xF8)}
      end
